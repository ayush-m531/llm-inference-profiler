"""
Per-layer channel ordering vs a single frozen order - second text.

Identical method to 07_per_layer_order.py, different perplexity text
(narrative/historical rather than technical prose).

Purpose: check which findings from 07 reproduce.
  - Does L0 still collapse under the frozen order? (expected: yes, it keeps 0%
    of the frozen top-5 channels, which is a structural fact about depth)
  - Do the L10/L11 gains (~3.3-3.7pp) reproduce?
  - Do the L4/L7 REGRESSIONS reproduce, or flip sign? If they flip, they were
    measurement noise.

Noise floor note: several perplexity "gains" in 07 were under 1pp, and some
compressed runs scored BETTER than uncompressed (impossible as a real effect),
indicating noise of roughly +/-0.5pp. Treat anything under ~1pp as
...
# narrative/historical text - same one used in 06_01
PPL_TEXT = ("The construction of the railway across the mountains took eleven years "
...
out(f"Text domain: narrative/historical (07 used technical prose)")
...
out("=" * 78)
out("NOTE: treat PPL gains under ~1pp as indistinguishable from noise.")
out()

results = {"model": MODEL_NAME, "calib_layer": CALIB_LAYER,
           "splits": SPLITS, "ref_perplexity": ref_ppl,
           "text_domain": "narrative",
...
for B in PPL_BUDGETS:
    ...
    for L in SPLITS:
        ...
        saved = (f_pick[1]["bytes"] - o_pick[1]["bytes"]) if (f_pick and o_pick) else 0
        mark = ""
        if f_lbl != o_lbl:
            mark = "  <-- CHANGED"
            total_changes += 1
...
if byte_savings:
    out(f"Savings where it changed: {min(byte_savings):.1f}% to "
        f"{max(byte_savings):.1f}% (mean {sum(byte_savings)/len(byte_savings):.1f}%).")
if regressions:
    out(f"REGRESSIONS (per-layer order was WORSE) at: {regressions}")
else:
    out("No regressions.")
out()
out("Compare against 07 (technical text) to see which findings reproduce:")
out("  - L0 collapse under the frozen order")
out("  - L10/L11 gains around 3-4pp")
out("  - L4/L7 regressions at the 1% budget (if these flip sign, they were noise)")
